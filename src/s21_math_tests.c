...
/* START_TEST(pow_test_1) {
  for (double k = -9; k <= 9; k += 1.7) {
    for (double g = -9; g < 9; g += 1.7) {
      ...
      if ((!is_nan(a) && !isnan(b)) && !(a == S21_INF && b == S21_INF) &&
          !(a == -S21_INF && b == -S21_INF)) {
...
START_TEST(pow_test_4) {
  long double a = s21_pow(-0.5, -S21_INF);
  // long double b = pow(-0.5, -S21_INF);
...
START_TEST(pow_test_6) {
  long double a = s21_pow(-S21_INF, -S21_INF);
  long double b = pow(-S21_INF, -S21_INF);
  ck_assert_ldouble_eq_tol(a, b, 1e-6);
}
END_TEST
...
START_TEST(fmod_ptest) {
  long double a = s21_fmod(S21_INF, S21_INF);
  // long double b = fmod(S21_INF , S21_INF);
...
START_TEST(log_test_5) {
  ...
  ck_assert_ldouble_eq_tol(log(S21_EXP), s21_log(S21_EXP), S21_EPS);
...
START_TEST(s21_pow_60) {
  ck_assert_ldouble_eq_tol(s21_pow(0.1, 0.000001), pow(0.1, 0.000001), S21_EPS);
}
END_TEST
